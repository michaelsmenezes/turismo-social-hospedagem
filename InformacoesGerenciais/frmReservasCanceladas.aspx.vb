...
Imports Turismo
Imports dtsReservasCanceladasTableAdapters

Partial Class InformacoesGerenciais_frmReservasCanceladas
    ...
    Protected Sub btnConsultar_Click(sender As Object, e As EventArgs) Handles btnConsultar.Click
        ...
        End If
        Params(1) = New ReportParameter("DataInicial", Format(CDate(txtDataInicial.Text), "yyyy-MM-dd"))
        Params(2) = New ReportParameter("Usuario", "Impresso por: " & User.Identity.Name.ToString.Replace("SESC-GO.COM.BR\", ""))
        Params(3) = New ReportParameter("DataFinal", Format(CDate(txtDataFinal.Text), "yyyy-MM-dd"))

        Dim ordersReservasCanceladasTableAdapter As DataTableReservasCanceladasTableAdapter = New DataTableReservasCanceladasTableAdapter
        ...
        If btnConsultar.Attributes.Item("AliasBancoTurismo") = "TurismoSocialCaldas" Then
            ...
            ordersReservasCanceladasTableAdapter.Connection.ConnectionString = oTurismo.strConnection.ConnectionString
        Else
            Dim oTurismo = New ConexaoDAO("TurismoSocialPiri")
            ordersReservasCanceladasTableAdapter.Connection.ConnectionString = oTurismo.strConnection.ConnectionString
        End If
        Dim ordersRelReservasCanceladas As System.Data.DataTable = ordersReservasCanceladasTableAdapter.GetData(Format(CDate(txtDataInicial.Text), "yyyy-MM-dd"), Format(CDate(txtDataFinal.Text), "yyyy-MM-dd"))
        Dim rdsReservasCanceladas As New ReportDataSource("dtsReservasCanceladas", ordersRelReservasCanceladas)
        'If ordersAniversarianteLinha.GetData(VarAniversariantes.ToString).Rows.Count > 0 Then
        '    'Chamando o relatório'
        '    'pnlGridCaldos.Visible = False
        '    rptReservasCanceladas.Visible = True
        '    rptReservasCanceladas.LocalReport.DataSources.Clear()
        '    rptReservasCanceladas.LocalReport.DataSources.Add(rdsRelAniversarianteLinha)
        '    rptReservasCanceladas.LocalReport.SetParameters(Params)
        'Else
        ...
        rptReservasCanceladas.Visible = True
        rptReservasCanceladas.LocalReport.DataSources.Clear()
        rptReservasCanceladas.LocalReport.DataSources.Add(rdsReservasCanceladas)
        rptReservasCanceladas.LocalReport.SetParameters(Params)
